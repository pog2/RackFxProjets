...
{
	// phases
	m_ModDelayLeft.m_uLFOPhase = 1; //quad
	m_ModDelayCenter.m_uLFOPhase = 0; //normal
	m_ModDelayRight.m_uLFOPhase = 1; //quad

	// LFO inversion (only the Right)
	m_ModDelayLeft.m_LFO.m_bInvert = false;
	m_ModDelayCenter.m_LFO.m_bInvert = false;
	m_ModDelayRight.m_LFO.m_bInvert = true; // *-1 
	...
	// Modulation types : chorus (2)
	m_ModDelayLeft.m_uModType = 2;
	m_ModDelayCenter.m_uModType = 2;
	m_ModDelayRight.m_uModType = 2;

	// Feedbacks
	m_ModDelayLeft.m_fFeedback_pct = m_fFeedback__pct_L;
	m_ModDelayCenter.m_fFeedback_pct = m_fFeedback__pct_C;
	m_ModDelayRight.m_fFeedback_pct = m_fFeedback__pct_R;

	// LFO Types : triangle (0)
	m_ModDelayLeft.m_uLFOType = 0;
	m_ModDelayCenter.m_uLFOType = 0;
	m_ModDelayRight.m_uLFOType = 0;
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CStereoLCRChorus::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
bool __stdcall CStereoLCRChorus::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	// (INSERT Effect)
	float fChorusLeftOut = 0.0;
	float fChorusCenterOut = 0.0;
	float fChorusRightOut = 0.0;
	...
	//sum Left & Right for enter C
	float fChorusCenterIn = 0.5*(fLeftIn + fRightIn);

	// use the chorus
	m_ModDelayLeft.processAudioFrame(&fLeftIn, &fChorusLeftOut, 1, 1);
	m_ModDelayCenter.processAudioFrame(&fChorusCenterIn, &fChorusCenterOut, 1, 1);
	m_ModDelayRight.processAudioFrame(&fRightIn, &fChorusRightOut, 1, 1);


	pOutputBuffer[0] = (fChorusLeftOut + fChorusCenterOut);
	...
	{
		pOutputBuffer[1] = (fChorusRightOut + fChorusCenterOut);
	}
...
-------------------------------------------------------------------
0				m_fModDepth_pct_L                 0
1				m_fModFrequency_Hz_L              1
2				m_fFeedback__pct_L                2
3				m_fModDepth_pct_C                 10
4				m_fModFrequency_Hz_C              11
5				m_fFeedback__pct_C                12
6				m_fModDepth_pct_R                 20
7				m_fModFrequency_Hz_R              21
8				m_fFeedback__pct_R                22
...
bool __stdcall CStereoLCRChorus::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CStereoLCRChorus::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CStereoLCRChorus::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
*/
void CStereoLCRChorus::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CStereoLCRChorus::showGUI(void* pInfo)
{
...
// **--0xDEA7--**
	...
	ui2->m_pUserCookedFloatData = &m_fFeedback__pct_L;
	ui2->m_pUserCookedDoubleData = NULL;
	ui2->m_pUserCookedUINTData = NULL;
	ui2->cControlUnits = "%";
	ui2->cVariableName = "m_fFeedback__pct_L";
	...
	ui5->m_pUserCookedFloatData = &m_fFeedback__pct_C;
	ui5->m_pUserCookedDoubleData = NULL;
	ui5->m_pUserCookedUINTData = NULL;
	ui5->cControlUnits = "%";
	ui5->cVariableName = "m_fFeedback__pct_C";
	...
	ui7->fInitUserFloatValue = 0.490000;
	...
	ui8->m_pUserCookedFloatData = &m_fFeedback__pct_R;
	ui8->m_pUserCookedDoubleData = NULL;
	ui8->m_pUserCookedUINTData = NULL;
	ui8->cControlUnits = "%";
	ui8->cVariableName = "m_fFeedback__pct_R";
