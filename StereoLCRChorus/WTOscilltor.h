...
class CWTOscilltor : public CPlugIn
{
public:
	...
	//Array for the LookupTable
	//normal
	float m_SinArray[1024];
	float m_SawtoothArray[1024];
	float m_TriangleArray[1024];
	float m_SquareArray[1024];
	//band limited (5 harmonics)
	float m_SinArray_BL5[1024];
	...
	{
		m_fReadIndex = 0.0;
		m_fQuadPhaseReadIndex = 256.0; //~pi/4 <-> 1/4 of buffer
	}
	// increment value
	float m_f_inc;
	//cooking function to uptade and compute variables
	void cookFrequency();
