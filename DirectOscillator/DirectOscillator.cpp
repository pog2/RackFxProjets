...
{
	//oscillation rate : theta = w/fs
	float f_wT = (2.0*pi*m_fFrequency_Hz / (float)m_nSampleRate);
	//coeff
	m_f_b1 = -2.0*cos(f_wT);
	m_f_b2 = 1.0;
	//init condition so that y[0] = 0
	// m_f_y_z1 = sin(-1.0*f_wT);
	// m_f_y_z2 = sin(-2.0*f_wT);

	//recalculate the init cond to have a continuity of sinus
	...
	//Calculate the (old) new sample
	m_f_y_z2 = sin(n*f_wT);
}
/* destructor()
	Destroy dynamically allocated variables
*/
CDirectOscillator::~CDirectOscillator(void)
{
...
*/
bool __stdcall CDirectOscillator::initialize()
{
	// Add your code here
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CDirectOscillator::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
*/
bool __stdcall CDirectOscillator::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
...
bool __stdcall CDirectOscillator::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CDirectOscillator::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CDirectOscillator::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
/* processAudioBuffer
	...
	******************************
	...
	m_HostProcessInfo.uTimeSigDenomintor = Host Time Signature Denominator (if supported by host)
*/
bool __stdcall CDirectOscillator::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CDirectOscillator::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CDirectOscillator::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CDirectOscillator::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CDirectOscillator::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CDirectOscillator::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CDirectOscillator::midiClock()
{
...
bool __stdcall CDirectOscillator::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
*/
void CDirectOscillator::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CDirectOscillator::showGUI(void* pInfo)
{
...
bool __stdcall CDirectOscillator::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! -------------------------------------- //
