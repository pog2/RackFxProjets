...
public:
	...
	//varialbes pour le buffer 
	float* m_pBuffer;
	int m_nReadIndex;
	int m_nWriteIndex;
	int m_nBufferSize;

	bool m_bUseExternalFeedBack = false; //Flag to allow user feedback
	float m_fFeedbackIn;
	// function to get current feedback
	float getCurrentFeedback() { return m_fFeedBack*m_pBuffer[m_nReadIndex]; };
	// function ti set the feedback sample
	void setCurrentFeedbackInput(float f) { m_fFeedBack = f; };
	//Enable / disable external FB source
	void setUsesExternalFeedback(bool l=false) { m_bUseExternalFeedBack = l; };
	//fonction pour mettre en forme des les variables issues des boutons pour l'ago
	void cookVariables();
	//fonction reset du buffer
	void resetDelay();
	// **--0x1A7F--**
	// ------------------------------------------------------------------------------- //
