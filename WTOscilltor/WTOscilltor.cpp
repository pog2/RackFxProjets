...
CWTOscilltor::CWTOscilltor()
{
	...
	for (i = 0; i < 1024; i++)
	{
		...
		//Calcul de la LUT y[n] = sin(2pi*f/1024*n)
		m_SinArray[i] = sin(2.0*pi /1024.0*(float)i);
		...
		{
			double n = double(g);
			m_SawtoothArray_BL5[i] += pow((float)-1.0, (float)(g+1))*
								  (1.0 /n)*
								  sin(2.0*pi*n*i/ 1024.0);
		}
		//triangle
		for (int g = 0; g <= 3; g++)
		{
		...
		{
			double n = double(g);
			m_SquareArray_BL5[i] += pow((float)-1.0, (float)n)*				
					sin(2.0*pi*n*i / 1024.0);
		}
...
void CWTOscilltor::cookFrequency()
{
	// inc = L*f/fs
...
void CWTOscilltor::doOscillate(float* pYn, float * pYqn)
{
	if (!m_bNoteOn)
	{
		*pYn = 0.0;
	...
	//get frac part of inc
	float fFrac = m_fReadIndex - nReadIndex;

	//setup second index for inter
	int nReadNextIndex = nReadIndex + 1 > 1023 ? 0 : nReadIndex + 1;
	int nQuadPhaseReadNextIndex = nQuadPhaseReadIndex + 1 > 1023 ? 0 : nQuadPhaseReadIndex + 1;
	//interpolate the output
	switch (m_uOscType)
	{
	case sine:
		fOutSample = dLinTerp(0, 1, m_SinArray[nReadIndex], m_SinArray[nReadNextIndex], fFrac);
		fQuadPhaseOutSample = dLinTerp(0, 1, m_SinArray[nQuadPhaseReadIndex], m_SinArray[nQuadPhaseReadIndex], fFrac);
		break;

	case saw:
		if (m_uTableMode == normal)
		{
			fOutSample = dLinTerp(0, 1, m_SawtoothArray[nReadIndex], m_SawtoothArray[nReadNextIndex], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_SawtoothArray[nQuadPhaseReadIndex],
				m_SawtoothArray[nQuadPhaseReadIndex], fFrac);
		}
		else
		{
			fOutSample = dLinTerp(0, 1, m_SawtoothArray_BL5[nReadIndex], m_SawtoothArray_BL5[nReadNextIndex], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_SawtoothArray_BL5[nQuadPhaseReadIndex], m_SawtoothArray_BL5[nQuadPhaseReadIndex], fFrac);
		}
		break;
	...
	case square:
		if (m_uTableMode == normal)
		{
			fOutSample = dLinTerp(0, 1, m_SquareArray[nReadIndex], m_SquareArray[nReadNextIndex], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_SquareArray[nQuadPhaseReadIndex], m_SquareArray[nQuadPhaseReadNextIndex], fFrac);
		}
		else
		{
			fOutSample = dLinTerp(0, 1, m_SquareArray_BL5[nReadIndex], m_SquareArray_BL5[nReadNextIndex], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_SquareArray_BL5[nQuadPhaseReadIndex], m_SquareArray_BL5[nQuadPhaseReadNextIndex], fFrac);
		}
		break;
	...
	case tri:
		if (m_uTableMode == normal)
		{
			fOutSample = dLinTerp(0, 1, m_TriangleArray[nReadIndex], m_TriangleArray[nReadNextIndex], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_TriangleArray[nQuadPhaseReadIndex], m_TriangleArray[nQuadPhaseReadNextIndex], fFrac);
		}
		else
		{
			fOutSample = dLinTerp(0, 1, m_TriangleArray_BL5[nReadIndex], m_TriangleArray_BL5[nReadNextIndex], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_TriangleArray_BL5[nQuadPhaseReadIndex], m_TriangleArray_BL5[nQuadPhaseReadNextIndex], fFrac);
	...
	default:
		//defautt = sin 
		fOutSample = dLinTerp(0, 1, m_SinArray[nReadIndex], m_SinArray[nReadNextIndex], fFrac);
		fQuadPhaseOutSample = dLinTerp(0, 1, m_SinArray[nQuadPhaseReadIndex], m_SinArray[nQuadPhaseReadNextIndex], fFrac);
		break;
	}
...
*/
CWTOscilltor::~CWTOscilltor(void)
{
...
*/
bool __stdcall CWTOscilltor::initialize()
{
	// Add your code here
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CWTOscilltor::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
*/
bool __stdcall CWTOscilltor::prepareForPlay()
{
	// Add your code here:
...
*/
bool __stdcall CWTOscilltor::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
...
//
bool __stdcall CWTOscilltor::userInterfaceChange(int nControlIndex)
{
...
bool __stdcall CWTOscilltor::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CWTOscilltor::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CWTOscilltor::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
/* processAudioBuffer
	...
	******************************
	...
	m_HostProcessInfo.uTimeSigDenomintor = Host Time Signature Denominator (if supported by host)
*/
bool __stdcall CWTOscilltor::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CWTOscilltor::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CWTOscilltor::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CWTOscilltor::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CWTOscilltor::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CWTOscilltor::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CWTOscilltor::midiClock()
{
...
bool __stdcall CWTOscilltor::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
*/
void CWTOscilltor::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CWTOscilltor::showGUI(void* pInfo)
{
...
bool __stdcall CWTOscilltor::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! -------------------------------------- //
...
// **--0xDEA7--**
	...
	ui0->dPresetData[0] = 440.000000;ui0->dPresetData[1] = 0.000000;ui0->dPresetData[2] = 0.000000;ui0->dPresetData[3] = 0.000000;ui0->dPresetData[4] = 0.000000;ui0->dPresetData[5] = 0.000000;ui0->dPresetData[6] = 0.000000;ui0->dPresetData[7] = 0.000000;ui0->dPresetData[8] = 0.000000;ui0->dPresetData[9] = 0.000000;ui0->dPresetData[10] = 0.000000;ui0->dPresetData[11] = 0.000000;ui0->dPresetData[12] = 0.000000;ui0->dPresetData[13] = 0.000000;ui0->dPresetData[14] = 0.000000;ui0->dPresetData[15] = 0.000000;
	...
	m_AssignButton1Name = "Start";
	m_AssignButton2Name = "Sop";
	m_AssignButton3Name = "";
